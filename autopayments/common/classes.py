...
from schemas.schemas import TonTransferResponse
...
class Wallet(Contract):
    ...
    def __init__(
        ...
        if mnemonics:
            mnemonics, _pub_k, _priv_k, wallet = Wallets.from_mnemonics(
                mnemonics, WalletVersionEnum(version), 0
            )
            self.mnemonics = mnemonics
            self.version = version
            self.address = self.provider._process_address(
                wallet.address.to_string()
            )
            self.full_data = True
        if not address and not mnemonics:
            mnemonics, _pub_k, _priv_k, wallet = Wallets.create(
                WalletVersionEnum(version), 0
            )
            self.mnemonics = mnemonics
            self.version = version
            self.address = self.provider._process_address(
    ...
    async def get_seqno(self):
        return await self.provider.get_wallet_seqno(
            self.address
        )
    ...
    async def transfer_ton(
        ...
        if not self.has_access():
            raise WalletError(
                'Cannot send tons from wallet without wallet mnemonics\n'
                'Create wallet like Wallet(mnemonics=["your", "mnemonic", "here"...], '
                'version="your_wallet_version")'
            )

        mnemonics, _pub_k, _priv_k, wallet = Wallets.from_mnemonics(
            self.mnemonics, WalletVersionEnum(self.version), 0
        )
